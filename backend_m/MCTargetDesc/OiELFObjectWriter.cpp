//===-- OiELFObjectWriter.cpp - Oi ELF Writer -------------------------===//
//
...
namespace {
  ...
  class OiELFObjectWriter : public MCELFObjectTargetWriter {
  public:
    OiELFObjectWriter(bool _is64Bit, uint8_t OSABI,
                        bool _isN64, bool IsLittleEndian);

    virtual ~OiELFObjectWriter();
...
OiELFObjectWriter::OiELFObjectWriter(bool _is64Bit, uint8_t OSABI,
                                         bool _isN64, bool IsLittleEndian)
  : MCELFObjectTargetWriter(_is64Bit, OSABI, ELF::EM_MIPS,
                            /*HasRelocationAddend*/ (_isN64) ? true : false,
                            /*IsN64*/ _isN64) {}

OiELFObjectWriter::~OiELFObjectWriter() {}

// FIXME: get the real EABI Version from the Subtarget class.
unsigned OiELFObjectWriter::getEFlags() const {
...
const MCSymbol *OiELFObjectWriter::ExplicitRelSym(const MCAssembler &Asm,
                                                    const MCValue &Target,
                                                    const MCFragment &F,
                                                    const MCFixup &Fixup,
                                                    bool IsPCRel) const {
...
unsigned OiELFObjectWriter::GetRelocType(const MCValue &Target,
                                           const MCFixup &Fixup,
                                           bool IsPCRel,
  ...
  case Oi::fixup_Oi_26:
    Type = ELF::R_MIPS_26;
    break;
  case Oi::fixup_Oi_CALL16:
    Type = ELF::R_MIPS_CALL16;
    break;
  case Oi::fixup_Oi_GOT_Global:
  ...
  case Oi::fixup_Oi_LO16:
    ...
    break;
  case Oi::fixup_Oi_TLSGD:
    Type = ELF::R_MIPS_TLS_GD;
    break;
  case Oi::fixup_Oi_GOTTPREL:
    Type = ELF::R_MIPS_TLS_GOTTPREL;
    break;
  case Oi::fixup_Oi_TPREL_HI:
    Type = ELF::R_MIPS_TLS_TPREL_HI16;
    break;
  case Oi::fixup_Oi_TPREL_LO:
    Type = ELF::R_MIPS_TLS_TPREL_LO16;
    break;
  case Oi::fixup_Oi_TLSLDM:
    Type = ELF::R_MIPS_TLS_LDM;
    break;
  case Oi::fixup_Oi_DTPREL_HI:
    Type = ELF::R_MIPS_TLS_DTPREL_HI16;
    break;
  case Oi::fixup_Oi_DTPREL_LO:
    Type = ELF::R_MIPS_TLS_DTPREL_LO16;
    break;
  case Oi::fixup_Oi_Branch_PCRel:
...
void OiELFObjectWriter::sortRelocs(const MCAssembler &Asm,
                                     std::vector<ELFRelocationEntry> &Relocs) {
...
MCObjectWriter *llvm::createOiELFObjectWriter(raw_ostream &OS,
                                                uint8_t OSABI,
                                                bool IsLittleEndian,
                                                bool Is64Bit) {
  MCELFObjectTargetWriter *MOTW = new OiELFObjectWriter(Is64Bit, OSABI,
                                                (Is64Bit) ? true : false,
                                                IsLittleEndian);
