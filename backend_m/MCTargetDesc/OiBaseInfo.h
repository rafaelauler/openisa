//===-- OiBaseInfo.h - Top level definitions for OI MC ------*- C++ -*-===//
//
...
// the Oi target useful for the compiler back-end and the MC libraries.
//
//===----------------------------------------------------------------------===//
#ifndef OIBASEINFO_H
#define OIBASEINFO_H
...
/// OiII - This namespace holds all of the target specific flags that
...
namespace OiII {
  ...
  enum TOF {
    //===------------------------------------------------------------------===//
    // Oi Specific MachineOperand flags.
...
OiGetSymAndOffset(const MCFixup &Fixup) {
  MCFixupKind FixupKind = Fixup.getKind();

  if ((FixupKind < FirstTargetFixupKind) ||
      (FixupKind >= MCFixupKind(Oi::LastTargetFixupKind)))
