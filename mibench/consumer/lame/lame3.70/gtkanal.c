...
int gtkmakeframe(void)
{
...
#ifndef HAVEMPGLIB
  printf("Error: GTK frame analyzer requires MPGLIB\n");
  exit(1);
#else
  ...
  if (gfp->input_format == sf_mp3) {
	...
	for ( j = 0; j < pinfo->framesize; j++ ) {
      ...
      if (mpglag == MAXMPGLAG) {
	printf("READ_AHEAD set too low - not enough frame buffering.\n");
	printf("MP3x display of input and output PCM data out of sync.\n");
      }
      else mpglag++; 
...
void plot_frame(void)
{
  ...
  if (i > MAXMPGLAG) {
    printf("input/output pcm syncing problem.  should not happen!\n");
    pplot2=pplot-1;
  }
