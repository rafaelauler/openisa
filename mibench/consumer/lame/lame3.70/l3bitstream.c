...
void III_format_bitstream(lame_global_flags *gfp, int bitsPerFrame,
  ...
  //  for (gr = 0; gr < MAX_GRANULES; gr++) {
  //for (ch = 0; ch < MAX_CHANNELS; ch++) {
  //      for (int j = 0; j < userSpectrumPH[gr][ch]->part->nrEntries; ++j) {
  //        printf("userSpectrumPH[%d][%d]->part->", gr, ch);
  //        printf("element[%d].value = %d", j,
  //               userSpectrumPH[gr][ch]->part->element[j].value);
  //        printf("  length = %d\n",
  //               userSpectrumPH[gr][ch]->part->element[j].length);
  //      }
  //    }
  //  }
...
static int encodeSideInfo(lame_global_flags *gfp, III_side_info_t *si) {
  ...
  if (gfp->error_protection) { /* (jo) error_protection: add crc16 information
                                  to header */
...
#ifdef DEBUG
    printf("## %d Huffman bits written (%02d + %02d), ", bitsWritten, bvbits,
           c1bits);
    printf("part2_length = %d, part2_3_length = %d, %d stuffing ##\n",
           gi->part2_length, gi->part2_3_length, stuffingBits);
#endif
}
