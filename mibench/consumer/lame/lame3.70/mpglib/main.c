...
unsigned long *num_samples)
{
  ...
  if (size>0 && !xing_header) {
    printf("Opps: first frame of mpglib output will be lost \n");
  }
...
int lame_decode_fromfile(FILE *fd, short pcm_l[], short pcm_r[])
{
  ...
  if (ret == MP3_OK) 
  {
    ...
    if ((outsize!=576) && (outsize!=1152)) {
      printf("Opps: mpg123 returned more than one frame!  Cant handle this... \n");
      exit(-50);
    }
...
int lame_decode(char *buf,int len,short pcm_l[],short pcm_r[])
{
  ...
  if (ret==MP3_OK) {
    ...
    if (outsize > 1152) {
      printf("Opps: mpg123 returned more than one frame!  shouldn't happen... \n");
      exit(-50);
    }
