 ...
 */
  ...
  if ((gfp->frameNum == 0) && (gr_out == 0)) {
    ...
    //    printf("numlines_s[0] = %d, Addr=%08X\n", numlines_s[0], &numlines_s[0]);
...
#define rpelev2 16
  ...
  for (chn = 0; chn < numchn; chn++) {
     ...
     **********************************************************************/
    for (j = cw_lower_index; j < cw_upper_index;
 ...
 **********************************************************************/
    ...
    for (sblock = 0; sblock < 3; sblock++) {
      ...
      for (b = 0; b < npart_s; b++) {
        ...
        //        printf("==============================\n");
        //        printf("b = %d s3ind_s[b][0] = %d s3ind_s[b][1] = %d",
        //               b, s3ind_s[b][0], s3ind_s[b][1]);
        //        printf("%dqthr_s[b] = %.20lf\n", 0, qthr_s[b]);
        //        printf("%decb = %.20lf\n", 0, ecb);
      ...
      for (sb = 0; sb < SBPSY_s; sb++) {
        ...
        //        printf("==============================\n");
        //        printf("bu_s[sb] = %d thr[bu_s[sb]] = %.20lf\n", bu_s[sb], thr[bu_s[sb]]);
        //        printf("bo_s[sb] = %d thr[bo_s[sb]] = %.20lf\n", bo_s[sb], thr[bo_s[sb]]);
        //        printf("chn = %d, sb = %d, ", chn, sb);
        //        printf("sblock = %d, val = %.20lf\n", sblock, thm[chn].s[sb][sblock]);
      }
    }
  } /* end loop over chn */
  ...
  for (loop = 0; loop < 6; loop++) {
    ...
    if (sfreq == freq_tp / freq_scale) {
      ...
      for (i = 0, k2 = 0; i < cbmax_tp; i++) {
        ...
        //        printf("numlines_l = %d, i = %d, numlines_l[i] = %d\n",
        //        (int)numlines_l, i, numlines_l[i]);
...
#define NEWBARKXXX
  ...
  for (loop = 0; loop < 6; loop++) {
    ...
    if (sfreq == freq_tp / freq_scale) {
      ...
      for (i = 0, k2 = 0; i < cbmax_tp; i++) {
        j = (int)*p++;
        //        printf("About to update numlines_s[%d] to %.20lf\n", i, *p);
        numlines_s[i] = (int)*p++;
        //        printf("  numlines_s[%d] = %d\n", i, numlines_s[i]);
        qthr_s[i] = *p++;
        /* norm_s[i] =*p++ */ p++;
        SNR[i] = *p++;
        bval_s[i] = *p++;
        //        printf("BTW, j equals to %d and i is %d\n", j, i);
        if (j != i) {
