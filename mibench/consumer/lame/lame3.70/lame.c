...
void dump_l3_side() {
  unsigned char *ptr = (char *) &l3_side;
  printf("Address = %08X\n", (int)ptr);
  for (int i = 0; i < 490; ++i) {
    if (i % 16 == 0)
      printf("%04X ", i);
...
void lame_init_params(lame_global_flags *gfp)
{
  ...
  */
  ...
  if (gfp->filter_type==0) {
	...
	printf("Warning: highpass filter disabled.  highpass frequency to small\n");
      }
...
void lame_print_config(lame_global_flags *gfp)
{
  ...
  if (gfp->resample_ratio!=1) {
    printf("Resampling:  input=%ikHz  output=%ikHz\n",
	    (int)in_samplerate,(int)out_samplerate);
  }
  if (gfp->highpass2>0.0)
    printf( "Using polyphase highpass filter, transition band: %.0f Hz -  %.0f Hz\n",
	    gfp->highpass1*out_samplerate*500,
	    gfp->highpass2*out_samplerate*500);
  if (gfp->lowpass1>0.0)
    printf( "Using polyphase lowpass filter,  transition band:  %.0f Hz - %.0f Hz\n",
  ...
  if (gfp->gtkflag) {
    ...
    puts("Encoding ");
    puts(strcmp(gfp->inPath, "-") ? gfp->inPath : "stdin");
    puts(" to ");
    puts(strcmp(gfp->outPath, "-") ? gfp->outPath : "stdout");
    puts("\n");
    if (gfp->VBR) {
      printf("Encoding as %.1fkHz VBR(q=%i)", gfp->out_samplerate / 1000.0,
             gfp->VBR_q);
      puts(mode_names[gfp->mode]);
      printf(" MPEG%i LayerIII  qval=%i\n", 2 - gfp->version, gfp->quality);
    } else {
      printf("%dEncoding as %.1f kHz ",0, gfp->out_samplerate / 1000.0);
      printf("%d kbps ", gfp->brate);
      puts(mode_names[gfp->mode]);
      printf("MPEG%i LayerIII ", 2 - gfp->version);
      printf("%d(%4.1fx)  qval=", gfp->mode, compression);
      printf("%i\n", gfp->quality);
    }
  }
  fflush(stderr);
