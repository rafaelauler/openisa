...
function run_test {
    echo "gtime -f'%e' -otimeoutput.txt adb -d shell ${REMOTEROOT}/"${@}"' &>/dev/null'" > run_test.sh
...
function run_family {
    run_test ${1}"-nat-arm "${2}" "${3}" "${4}
    echo -ne " "

    run_test ${1}"-nolocals "${2}" "${3}" "${4}
    echo -ne " "

    run_test ${1}"-locals "${2}" "${3}" "${4}
    echo -ne " "

    run_test ${1}"-oneregion "${2}" "${3}" "${4}
    echo -ne " "
