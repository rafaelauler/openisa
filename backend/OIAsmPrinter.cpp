//===-- OIAsmPrinter.cpp - OI LLVM assembly writer ------------------===//
//
...
namespace {
  class OIAsmPrinter : public AsmPrinter {
  public:
    explicit OIAsmPrinter(TargetMachine &TM, MCStreamer &Streamer)
      : AsmPrinter(TM, Streamer) {}

    virtual const char *getPassName() const {
      return "OI Assembly Printer";
    }
...
void OIAsmPrinter::printMemOperand(const MachineInstr *MI, int opNum,
...
bool OIAsmPrinter::printGetPCX(const MachineInstr *MI, unsigned opNum,
                                  raw_ostream &O) {
  std::string operand = "";
  const MachineOperand &MO = MI->getOperand(opNum);
  switch (MO.getType()) {
...
void OIAsmPrinter::printCCOperand(const MachineInstr *MI, int opNum,
                                     raw_ostream &O) {
  int CC = (int)MI->getOperand(opNum).getImm();
  O << OICondCodeToString((SPCC::CondCodes)CC);
}

/// PrintAsmOperand - Print out an operand for an inline asm expression.
///
bool OIAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
                                      unsigned AsmVariant,
...
extern "C" void LLVMInitializeOIAsmPrinter() { 
  ...
  RegisterAsmPrinter<OIAsmPrinter> Y(TheOIV9Target);
}
