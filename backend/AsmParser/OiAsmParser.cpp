...
static const MCInstrDesc &getInstDesc(unsigned Opcode) {
  return OiInsts[Opcode];
}

bool OiAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
                                       SmallVectorImpl<MCInst> &Instructions) {
  ...
  if (MCID.hasDelaySlot() && Options.isReorder()) {
    ...
    MCInst NopInst;
    NopInst.setOpcode(Oi::SLL);
    NopInst.addOperand(MCOperand::CreateReg(Oi::ZERO));
    NopInst.addOperand(MCOperand::CreateReg(Oi::ZERO));
...
void OiAsmParser::expandLoadImm(MCInst &Inst, SMLoc IDLoc,
  ...
  if (0 <= ImmValue && ImmValue <= 65535) {
    ...
    tmpInst.addOperand(MCOperand::CreateReg(Oi::ZERO));
...
void OiAsmParser::expandMemInst(MCInst &Inst, SMLoc IDLoc,
  ...
  unsigned TmpRegNum;
  unsigned AtRegNum = getReg((isOi64()) ? Oi::CPU64RegsRegClassID
                             : Oi::CPURegsRegClassID, getATReg());
...
int OiAsmParser::matchCPURegisterName(StringRef Name) {
   int CC;
...
int OiAsmParser::matchRegisterName(StringRef Name, bool is64BitReg) {
  ...
  if (CC != -1)
    return matchRegisterByNumber(CC, is64BitReg ? Oi::CPU64RegsRegClassID
...
int OiAsmParser::tryParseRegister(bool is64BitReg) {
  ...
  if (Tok.is(AsmToken::Identifier)) {
    ...
    RegNum = matchRegisterByNumber(static_cast<unsigned>(Tok.getIntVal()),
        is64BitReg ? Oi::CPU64RegsRegClassID : Oi::CPURegsRegClassID);
  return RegNum;
}

bool OiAsmParser::tryParseRegisterOperand(
...
bool OiAsmParser::ParseOperand(SmallVectorImpl<MCParsedAsmOperand*>&Operands,
  ...
  case AsmToken::Identifier:
    ...
    if (searchSymbolAlias(Operands, OiOperand::Kind_None))
      return false;
...
const MCExpr* OiAsmParser::evaluateRelocExpr(const MCExpr *Expr,
                                               StringRef RelocStr) {
  const MCExpr *Res;
...
bool OiAsmParser::isEvaluated(const MCExpr *Expr) {
...
bool OiAsmParser::parseMemOffset(const MCExpr *&Res, bool isParenExpr) {
  SMLoc S;
  bool Result = true;
  ...
  if (getLexer().getKind() != AsmToken::Dollar) {
    ...
    if (Tok.isNot(AsmToken::LParen)) {
      ...
      if (Tok.is(AsmToken::EndOfStatement)) {
        ...
        Operands.push_back(OiOperand::CreateMem(isOi64() ? Oi::ZERO_64
                                                             : Oi::ZERO,
                           IdVal, S, E));
        return MatchOperand_Success;
      }
...
OiAsmParser::parseCPU64Regs(SmallVectorImpl<MCParsedAsmOperand*> &Operands) {
  ...
  if (getLexer().getKind() == AsmToken::Identifier) {
    if (searchSymbolAlias(Operands, OiOperand::Kind_CPU64Regs))
      return MatchOperand_Success;
    return MatchOperand_NoMatch;
  }
...
bool OiAsmParser::searchSymbolAlias(
  ...
  if (Sym) {
    ...
    if (Expr->getKind() == MCExpr::SymbolRef) {
      ...
      if (DefSymbol.startswith("$")) {
        ...
        int RegNum = matchRegisterName(DefSymbol.substr(1), isOi64());
        if (RegNum > -1) {
          Parser.Lex();
          OiOperand *op = OiOperand::CreateReg(RegNum, S,
                                                   Parser.getTok().getLoc());
          op->setRegKind((OiOperand::RegisterKind) RegisterKind);
      ...
      OiOperand *op = OiOperand::CreateImm(Const, S,
          Parser.getTok().getLoc());
      Operands.push_back(op);
      return true;
    }
...
OiAsmParser::parseCPURegs(SmallVectorImpl<MCParsedAsmOperand*> &Operands) {
  ...
  if (getLexer().getKind() == AsmToken::Identifier) {
    if (searchSymbolAlias(Operands, OiOperand::Kind_CPURegs))
      return MatchOperand_Success;
    return MatchOperand_NoMatch;
  }
  // If the first token is not '$' we have an error.
      ...
      if (Dot == StringRef::npos) // Only one '.' in a string, it's a format.
        ...
        if (Name.startswith("c.")) {
          ...
          SMLoc E = SMLoc::getFromPointer(
              Parser.getTok().getLoc().getPointer() - 1);
          Operands.push_back(
              OiOperand::CreateImm(MCConstantExpr::Create(Cc, getContext()),
                                     NameLoc, E));
        } else {
          // trunc, ceil, floor ...
...
bool OiAsmParser::parseSetAssignment() {
  StringRef Name;
  const MCExpr *Value;
