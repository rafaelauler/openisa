//===-- OiSEFrameLowering.cpp - Oi32/64 Frame Information -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Oi32/64 implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
ExpandPseudo::ExpandPseudo(MachineFunction &MF_)
  : MF(MF_),
    TII(*static_cast<const OiSEInstrInfo*>(MF.getTarget().getInstrInfo())),
...
bool ExpandPseudo::expandInstr(MachineBasicBlock &MBB, Iter I) {
  switch(I->getOpcode()) {
  case Oi::LOAD_CCOND_DSP:
  case Oi::LOAD_CCOND_DSP_P8:
    expandLoadCCond(MBB, I);
    break;
  case Oi::STORE_CCOND_DSP:
  case Oi::STORE_CCOND_DSP_P8:
    expandStoreCCond(MBB, I);
    break;
  case Oi::LOAD_AC64:
  case Oi::LOAD_AC64_P8:
  case Oi::LOAD_AC_DSP:
  case Oi::LOAD_AC_DSP_P8:
    expandLoadACC(MBB, I, 4);
    break;
  case Oi::LOAD_AC128:
  case Oi::LOAD_AC128_P8:
    expandLoadACC(MBB, I, 8);
    break;
  case Oi::STORE_AC64:
  case Oi::STORE_AC64_P8:
  case Oi::STORE_AC_DSP:
  case Oi::STORE_AC_DSP_P8:
    expandStoreACC(MBB, I, 4);
    break;
  case Oi::STORE_AC128:
  case Oi::STORE_AC128_P8:
...
void ExpandPseudo::expandLoadACC(MachineBasicBlock &MBB, Iter I,
  ...
  unsigned Dst = I->getOperand(0).getReg(), FI = I->getOperand(1).getIndex();
  unsigned Lo = RegInfo.getSubReg(Dst, Oi::sub_lo);
  unsigned Hi = RegInfo.getSubReg(Dst, Oi::sub_hi);
...
void ExpandPseudo::expandStoreACC(MachineBasicBlock &MBB, Iter I,
  ...
  unsigned Lo = RegInfo.getSubReg(Src, Oi::sub_lo);
  unsigned Hi = RegInfo.getSubReg(Src, Oi::sub_hi);
  DebugLoc DL = I->getDebugLoc();
...
bool ExpandPseudo::expandCopy(MachineBasicBlock &MBB, Iter I) {
  ...
  if (Oi::ACRegsDSPRegClass.contains(Dst, Src))
    return expandCopyACC(MBB, I, Dst, Src, 4);

  if (Oi::ACRegs128RegClass.contains(Dst, Src))
...
bool ExpandPseudo::expandCopyACC(MachineBasicBlock &MBB, Iter I, unsigned Dst,
  ...
  unsigned DstLo = RegInfo.getSubReg(Dst, Oi::sub_lo);
  unsigned DstHi = RegInfo.getSubReg(Dst, Oi::sub_hi);
  unsigned SrcLo = RegInfo.getSubReg(Src, Oi::sub_lo);
  unsigned SrcHi = RegInfo.getSubReg(Src, Oi::sub_hi);
  DebugLoc DL = I->getDebugLoc();
...
unsigned OiSEFrameLowering::ehDataReg(unsigned I) const {
...
void OiSEFrameLowering::emitPrologue(MachineFunction &MF) const {
  MachineBasicBlock &MBB   = MF.front();
  MachineFrameInfo *MFI    = MF.getFrameInfo();
  OiFunctionInfo *OiFI = MF.getInfo<OiFunctionInfo>();
  const OiRegisterInfo *RegInfo =
    static_cast<const OiRegisterInfo*>(MF.getTarget().getRegisterInfo());
  const OiSEInstrInfo &TII =
    *static_cast<const OiSEInstrInfo*>(MF.getTarget().getInstrInfo());
  MachineBasicBlock::iterator MBBI = MBB.begin();
  DebugLoc dl = MBBI != MBB.end() ? MBBI->getDebugLoc() : DebugLoc();
  unsigned SP = STI.isABI_N64() ? Oi::SP_64 : Oi::SP;
  unsigned FP = STI.isABI_N64() ? Oi::FP_64 : Oi::FP;
  ...
  if (CSI.size()) {
    ...
    for (std::vector<CalleeSavedInfo>::const_iterator I = CSI.begin(),
      ...
      if (Oi::AFGR64RegClass.contains(Reg)) {
        ...
        MachineLocation SrcML0(RegInfo->getSubReg(Reg, Oi::sub_fpeven));
        MachineLocation SrcML1(RegInfo->getSubReg(Reg, Oi::sub_fpodd));
  ...
  if (OiFI->callsEhReturn()) {
    ...
    for (int I = 0; I < 4; ++I) {
      ...
      TII.storeRegToStackSlot(MBB, MBBI, ehDataReg(I), false,
                              OiFI->getEhDataRegFI(I), RC, RegInfo);
    }
    ...
    for (int I = 0; I < 4; ++I) {
      int64_t Offset = MFI->getObjectOffset(OiFI->getEhDataRegFI(I));
      DstML = MachineLocation(MachineLocation::VirtualFP, Offset);
...
void OiSEFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (OiFI->callsEhReturn()) {
    ...
    for (int J = 0; J < 4; ++J) {
      TII.loadRegFromStackSlot(MBB, I, ehDataReg(J), OiFI->getEhDataRegFI(J),
                               RC, RegInfo);
    }
  }
...
spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  for (unsigned i = 0, e = CSI.size(); i != e; ++i) {
    ...
    bool IsRAAndRetAddrIsTaken = (Reg == Oi::RA || Reg == Oi::RA_64)
...
bool
OiSEFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
...
processFunctionBeforeCalleeSavedScan(MachineFunction &MF,
  ...
  if (OiFI->callsEhReturn())
    OiFI->createEhDataRegsFI();

  // Expand pseudo instructions which load, store or copy accumulators.
  // Add an emergency spill slot if a pseudo was expanded.
  if (ExpandPseudo(MF).expand()) {
    // The spill slot should be half the size of the accumulator. If target is
    // oi64, it should be 64-bit, otherwise it should be 32-bt.
    const TargetRegisterClass *RC = STI.hasOi64() ?
      &Oi::CPU64RegsRegClass : &Oi::CPURegsRegClass;
  ...
  uint64_t MaxSPOffset = MF.getInfo<OiFunctionInfo>()->getIncomingArgSize() +
...
llvm::createOiSEFrameLowering(const OiSubtarget &ST) {
  return new OiSEFrameLowering(ST);
}
