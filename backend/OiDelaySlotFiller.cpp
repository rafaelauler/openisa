//===-- OiDelaySlotFiller.cpp - Oi Delay Slot Filler ------------------===//
//
...
static cl::opt<bool> DisableForwardSearch(
  "disable-oi-df-forward-search",
  cl::init(true),
  cl::desc("Disallow OI delay filler to search forward."),
  cl::Hidden);

static cl::opt<bool> DisableSuccBBSearch(
  "disable-oi-df-succbb-search",
  cl::init(true),
  cl::desc("Disallow OI delay filler to search successor basic blocks."),
  cl::Hidden);

static cl::opt<bool> DisableBackwardSearch(
  "disable-oi-df-backward-search",
  cl::init(false),
  cl::desc("Disallow OI delay filler to search backward."),
  cl::Hidden);
...
namespace {
  ...
  private:
    ...
    std::pair<OiInstrInfo::BranchType, MachineInstr *>
...
void RegDefsUses::setCallerSaved(const MachineInstr &MI) {
  ...
  CallerSavedRegs.reset(Oi::ZERO);
  CallerSavedRegs.reset(Oi::ZERO_64);
...
void RegDefsUses::setUnallocatableRegs(const MachineFunction &MF) {
  ...
  AllocSet.set(Oi::ZERO);
  AllocSet.set(Oi::ZERO_64);
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  ...
  for (Iter I = MBB.begin(); I != MBB.end(); ++I) {
    ...
    BuildMI(MBB, llvm::next(I), I->getDebugLoc(), TII->get(Oi::NOP));
...
/// createOiDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Oi MachineFunctions
FunctionPass *llvm::createOiDelaySlotFillerPass(OiTargetMachine &tm) {
...
Filler::getBranch(MachineBasicBlock &MBB, const MachineBasicBlock &Dst) const {
  const OiInstrInfo *TII =
    static_cast<const OiInstrInfo*>(TM.getInstrInfo());
  MachineBasicBlock *TrueBB = 0, *FalseBB = 0;
  SmallVector<MachineInstr*, 2> BranchInstrs;
  SmallVector<MachineOperand, 2> Cond;

  OiInstrInfo::BranchType R =
    TII->AnalyzeBranch(MBB, TrueBB, FalseBB, Cond, false, BranchInstrs);

  if ((R == OiInstrInfo::BT_None) || (R == OiInstrInfo::BT_NoBranch))
    return std::make_pair(R, (MachineInstr*)NULL);

  if (R != OiInstrInfo::BT_CondUncond) {
    if (!hasUnoccupiedSlot(BranchInstrs[0]))
      return std::make_pair(OiInstrInfo::BT_None, (MachineInstr*)NULL);

    assert(((R != OiInstrInfo::BT_Uncond) || (TrueBB == &Dst)));
  ...
  if (hasUnoccupiedSlot(BranchInstrs[0]))
    return std::make_pair(OiInstrInfo::BT_Cond, BranchInstrs[0]);

  // If that fails, try the unconditional branch.
  if (hasUnoccupiedSlot(BranchInstrs[1]) && (FalseBB == &Dst))
    return std::make_pair(OiInstrInfo::BT_Uncond, BranchInstrs[1]);
...
bool Filler::examinePred(MachineBasicBlock &Pred, const MachineBasicBlock &Succ,
                         RegDefsUses &RegDU, bool &HasMultipleSuccs,
                         BB2BrMap &BrMap) const {
  std::pair<OiInstrInfo::BranchType, MachineInstr *> P =
    getBranch(Pred, Succ);

  // Return if either getBranch wasn't able to analyze the branches or there
  // were no branches with unoccupied slots.
  if (P.first == OiInstrInfo::BT_None)
    return false;

  if ((P.first != OiInstrInfo::BT_Uncond) &&
      (P.first != OiInstrInfo::BT_NoBranch)) {
