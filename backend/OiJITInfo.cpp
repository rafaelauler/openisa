//===-- OiJITInfo.cpp - Implement the Oi JIT Interface ----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the JIT interfaces for the Oi target.
//
//===----------------------------------------------------------------------===//
...
void OiJITInfo::replaceMachineCodeForFunction(void *Old, void *New) {
  ...
  if ((NewAddr & 0xF0000000) == ((OldAddr + 4) & 0xF0000000)) {
    ...
    *OldInstruction = NopInstr;
    ...
    if ((CurrInstrHintClear != ReturnSequence) &&
      ...
      // Unsupported case
      report_fatal_error("OiJITInfo::replaceMachineCodeForFunction");
    }
  }
}
...
// OiCompilationCallbackC and restores registers.
extern "C" {
#if defined (__oi__)
void OiCompilationCallback();

  asm(
    ".text\n"
    ".align 2\n"
    ".globl " ASMPREFIX "OiCompilationCallback\n"
    ASMPREFIX "OiCompilationCallback:\n"
    ".ent " ASMPREFIX "OiCompilationCallback\n"
    ".frame  $sp, 32, $ra\n"
    ".set  noreorder\n"
    ".cpload $t9\n"
    ...
    // to the OiCompilationCallbackC.
    "addiu $a0, $t8, -16\n"
    "jal " ASMPREFIX "OiCompilationCallbackC\n"
    "nop\n"
    ...
    ".set  reorder\n"
    ".end " ASMPREFIX "OiCompilationCallback\n"
      );
#else  // host != Oi
  void OiCompilationCallback() {
    llvm_unreachable(
      "Cannot call OiCompilationCallback() on a non-Oi arch!");
  }
#endif
}

/// OiCompilationCallbackC - This is the target-specific function invoked
/// by the function stub when we did not know the real target of a call.
/// This function must locate the start of the stub or call site and pass
/// it into the JIT compiler function.
extern "C" void OiCompilationCallbackC(intptr_t StubAddr) {
...
TargetJITInfo::LazyResolverFn OiJITInfo::getLazyResolverFunction(
    JITCompilerFn F) {
  JITCompilerFunction = F;
  return OiCompilationCallback;
}

TargetJITInfo::StubLayout OiJITInfo::getStubLayout() {
...
void *OiJITInfo::emitFunctionStub(const Function *F, void *Fn,
                                    JITCodeEmitter &JCE) {
  JCE.emitAlignment(4);
  ...
  if (Fn != (void*)(intptr_t)OiCompilationCallback)
    EmittedAddr = (intptr_t)Fn;
  else
    EmittedAddr = (intptr_t)OiCompilationCallback;
...
void OiJITInfo::relocate(void *Function, MachineRelocation *MR,
  ...
  for (unsigned i = 0; i != NumRelocs; ++i, ++MR) {
    ...
    switch ((Oi::RelocationType) MR->getRelocationType()) {
    case Oi::reloc_oi_pc16:
