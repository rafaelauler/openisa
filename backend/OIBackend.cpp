//===-- OIBackend.cpp - Library for converting LLVM code to C++ code -----===//
//
...
FuncName("oifname", cl::desc("Specify the name of the generated function"),
...
static cl::opt<WhatToGenerate> GenerationType("oi", cl::Optional,
...
static cl::opt<std::string> NameToGenerate("oifor", cl::Optional,
  cl::desc("Specify the name of the thing to generate"),
  cl::init("!bad!"));

extern "C" void LLVMInitializeOIBackendTarget() {
  // Register the target.
  RegisterTargetMachine<OITargetMachine> X(OIBackendTarget);
}
...
namespace {
  ...
  /// OIWriter - This class is the main chunk of code that converts an LLVM
  /// module to a C++ translation unit.
  class OIWriter : public ModulePass {
  ...
  public:
    static char ID;
    explicit OIWriter(formatted_raw_ostream &o) :
  ...
  private:
    ...
    std::string getOIName(Type* val);
    inline void printOIName(Type* val);

    std::string getOIName(const Value* val);
    inline void printOIName(const Value* val);
...
formatted_raw_ostream &OIWriter::nl(formatted_raw_ostream &Out, int delta) {
  Out << '\n';
...
void OIWriter::error(const std::string& msg) {
...
void OIWriter::printCFP(const ConstantFP *CFP) {
  bool ignored;
...
void OIWriter::printCallingConv(CallingConv::ID cc){
...
void OIWriter::printLinkageType(GlobalValue::LinkageTypes LT) {
  switch (LT) {
...
void OIWriter::printVisibilityType(GlobalValue::VisibilityTypes VisType) {
  switch (VisType) {
...
void OIWriter::printThreadLocalMode(GlobalVariable::ThreadLocalMode TLM) {
  switch (TLM) {
...
void OIWriter::printEscapedString(const std::string &Str) {
...
std::string OIWriter::getOIName(Type* Ty) {
...
void OIWriter::printOIName(Type* Ty) {
  printEscapedString(getOIName(Ty));
}

std::string OIWriter::getOIName(const Value* val) {
  std::string name;
...
void OIWriter::printOIName(const Value* val) {
  printEscapedString(getOIName(val));
}

void OIWriter::printAttributes(const AttrListPtr &PAL,
...
void OIWriter::printType(Type* Ty) {
  ...
  std::string typeName(getOIName(Ty));
  ...
  case Type::FunctionTyID:  {
    ...
    for (; PI != PE; ++PI) {
      ...
      printType(argTy);
      std::string argName(getOIName(argTy));
      Out << typeName << "_args.push_back(" << argName;
      Out << ");";
      nl(Out);
    }
    printType(FT->getReturnType());
    std::string retTypeName(getOIName(FT->getReturnType()));
  ...
  case Type::StructTyID: {
    ...
    for (; EI != EE; ++EI) {
      ...
      printType(fieldTy);
      std::string fieldName(getOIName(fieldTy));
  ...
  case Type::ArrayTyID: {
    ...
    printType(ET);
    if (DefinedTypes.find(Ty) == DefinedTypes.end()) {
      std::string elemName(getOIName(ET));
...
void OIWriter::printTypes(const Module* M) {
...
void OIWriter::printConstant(const Constant *CV) {
  ...
  std::string constName(getOIName(CV));
  std::string typeName(getOIName(CV->getType()));
  ...
  if (const ConstantInt *CI = dyn_cast<ConstantInt>(CV)) {
    ...
    if (CDS->isString()) {
      ...
      for (unsigned i = 0; i != CDS->getNumElements(); ++i) {
        ...
        printConstant(Elt);
        Out << constName << "_elems.push_back(" << getOIName(Elt) << ");";
        nl(Out);
      }
      Out << "Constant* " << constName;
    ...
    if (CE->getOpcode() == Instruction::GetElementPtr) {
      ...
      for (unsigned i = 1; i < CE->getNumOperands(); ++i ) {
        ...
        Out << constName << "_indices.push_back("
            << getOIName(CE->getOperand(i)) << ");";
        nl(Out);
      }
      Out << "Constant* " << constName
          << " = ConstantExpr::getGetElementPtr("
          << getOIName(CE->getOperand(0)) << ", "
          << constName << "_indices);";
    } else if (CE->isCast()) {
      printConstant(CE->getOperand(0));
      ...
      Out << ", " << getOIName(CE->getOperand(0)) << ", "
          << getOIName(CE->getType()) << ");";
    } else {
...
void OIWriter::printConstants(const Module* M) {
...
void OIWriter::printVariableUses(const GlobalVariable *GV) {
...
void OIWriter::printVariableHead(const GlobalVariable *GV) {
  nl(Out) << "GlobalVariable* " << getOIName(GV);
  if (is_inline) {
    Out << " = mod->getGlobalVariable(mod->getContext(), ";
    printEscapedString(GV->getName());
    Out << ", " << getOIName(GV->getType()->getElementType()) << ",true)";
    nl(Out) << "if (!" << getOIName(GV) << ") {";
    in(); nl(Out) << getOIName(GV);
  }
  Out << " = new GlobalVariable(/*Module=*/*mod, ";
  nl(Out) << "/*Type=*/";
  printOIName(GV->getType()->getElementType());
  Out << ",";
...
void OIWriter::printVariableBody(const GlobalVariable *GV) {
  if (GV->hasInitializer()) {
    printOIName(GV);
    Out << "->setInitializer(";
    Out << getOIName(GV->getInitializer()) << ");";
    nl(Out);
  }
}

std::string OIWriter::getOpName(const Value* V) {
...
void OIWriter::printInstruction(const Instruction *I,
                                 const std::string& bbname) {
  std::string iName(getOIName(I));
  ...
  case Instruction::Alloca: {
    ...
    Out << "AllocaInst* " << iName << " = new AllocaInst("
        << getOIName(allocaI->getAllocatedType()) << ", ";
  ...
  case Instruction::Call: {
    ...
    if (const InlineAsm* ila = dyn_cast<InlineAsm>(call->getCalledValue())) {
      Out << "InlineAsm* " << getOIName(ila) << " = InlineAsm::get("
          << getOIName(ila->getFunctionType()) << ", \""
          << ila->getAsmString() << "\", \""
  ...
  case Instruction::Select: {
    ...
    Out << "SelectInst* " << getOIName(sel) << " = SelectInst::Create(";
  ...
  case Instruction::VAArg: {
    ...
    Out << "VAArgInst* " << getOIName(va) << " = new VAArgInst("
        << opNames[0] << ", " << getOIName(va->getType()) << ", \"";
  ...
  case Instruction::ExtractElement: {
    ...
    Out << "ExtractElementInst* " << getOIName(eei)
  ...
  case Instruction::InsertElement: {
    ...
    Out << "InsertElementInst* " << getOIName(iei)
  ...
  case Instruction::ShuffleVector: {
    ...
    Out << "ShuffleVectorInst* " << getOIName(svi)
  ...
  case Instruction::ExtractValue: {
    ...
    Out << "ExtractValueInst* " << getOIName(evi)
  ...
  case Instruction::InsertValue: {
    ...
    Out << "InsertValueInst* " << getOIName(ivi)
...
void OIWriter::printFunctionUses(const Function* F) {
...
void OIWriter::printFunctionHead(const Function* F) {
  nl(Out) << "Function* " << getOIName(F);
  Out << " = mod->getFunction(\"";
  printEscapedString(F->getName());
  Out << "\");";
  nl(Out) << "if (!" << getOIName(F) << ") {";
  nl(Out) << getOIName(F);

  Out<< " = Function::Create(";
  nl(Out,1) << "/*Type=*/" << getOIName(F->getFunctionType()) << ",";
  nl(Out) << "/*Linkage=*/";
  ...
  Out << "}";
  nl(Out);
  printAttributes(F->getAttributes(), getOIName(F));
  printOIName(F);
  Out << "->setAttributes(" << getOIName(F) << "_PAL);";
  nl(Out);
}

void OIWriter::printFunctionBody(const Function *F) {
  if (F->isDeclaration())
  ...
  if (!is_inline) {
    if (!F->arg_empty()) {
      Out << "Function::arg_iterator args = " << getOIName(F)
          << "->arg_begin();";
      nl(Out);
    }
    for (Function::const_arg_iterator AI = F->arg_begin(), AE = F->arg_end();
         AI != AE; ++AI) {
      Out << "Value* " << getOIName(AI) << " = args++;";
      nl(Out);
      if (AI->hasName()) {
        Out << getOIName(AI) << "->setName(\"";
  ...
  for (Function::const_iterator BI = F->begin(), BE = F->end();
       BI != BE; ++BI) {
    std::string bbname(getOIName(BI));
    Out << "BasicBlock* " << bbname <<
           " = BasicBlock::Create(mod->getContext(), \"";
    if (BI->hasName())
      printEscapedString(BI->getName());
    Out << "\"," << getOIName(BI->getParent()) << ",0);";
    nl(Out);
  }
  ...
  while (!ForwardRefs.empty()) {
    ...
    Out << I->second << "->replaceAllUsesWith("
        << getOIName(I->first) << "); delete " << I->second << ";";
    nl(Out);
    ForwardRefs.erase(I);
  }
}

void OIWriter::printInline(const std::string& fname,
  ...
  printFunctionBody(F);
  is_inline = false;
  Out << "return " << getOIName(F->begin()) << ";";
  nl(Out) << "}";
  nl(Out);
}

void OIWriter::printModuleBody() {
  ...
  for (Module::const_iterator I = TheModule->begin(), E = TheModule->end();
       I != E; ++I) {
    if (!I->isDeclaration()) {
      nl(Out) << "// Function: " << I->getName() << " (" << getOIName(I)
              << ")";
      nl(Out) << "{";
      nl(Out,1);
      printFunctionBody(I);
...
void OIWriter::printProgram(const std::string& fname,
...
void OIWriter::printModule(const std::string& fname,
                            const std::string& mName) {
  nl(Out) << "Module* " << fname << "() {";
...
void OIWriter::printContents(const std::string& fname,
                              const std::string& mName) {
...
void OIWriter::printFunction(const std::string& fname,
  ...
  printFunctionHead(F);
  printFunctionBody(F);
  Out << "return " << getOIName(F) << ";\n";
  Out << "}\n";
}

void OIWriter::printFunctions() {
...
void OIWriter::printVariable(const std::string& fname,
  ...
  printVariableHead(GV);
  printVariableBody(GV);
  Out << "return " << getOIName(GV) << ";\n";
  Out << "}\n";
}

void OIWriter::printType(const std::string &fname,
  ...
  printType(Ty);
  Out << "return " << getOIName(Ty) << ";\n";
  Out << "}\n";
}

bool OIWriter::runOnModule(Module &M) {
  TheModule = &M;
...
bool OITargetMachine::addPassesToEmitFile(PassManagerBase &PM,
