//===-- OiInstPrinter.cpp - Convert Oi MCInst to assembly syntax ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an Oi MCInst to a .s file.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "asm-printer"
#include "OiInstPrinter.h"
...
const char* Oi::OiFCCToString(Oi::CondCode CC) {
  switch (CC) {
  case FCOND_F:
...
void OiInstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const {
  OS << '$' << StringRef(getRegisterName(RegNo)).lower();
}

void OiInstPrinter::printInst(const MCInst *MI, raw_ostream &O,
                                StringRef Annot) {
  switch (MI->getOpcode()) {
  default:
    break;
...
void OiInstPrinter::printCPURegs(const MCInst *MI, unsigned OpNo,
                                   raw_ostream &O) {
  printRegName(O, MI->getOperand(OpNo).getReg());
}

void OiInstPrinter::printOperand(const MCInst *MI, unsigned OpNo,
                                   raw_ostream &O) {
...
void OiInstPrinter::printUnsignedImm(const MCInst *MI, int opNum,
                                       raw_ostream &O) {
