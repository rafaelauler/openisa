//===-- OiAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
OiAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void OiAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void OiAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x80000000ULL) & 0xffffffff00000000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffffffULL));
}

void OiAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffff00000000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffffffULL));
}

void OiAnalyzeImmediate::GetInstSeqLsSLL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void OiAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void OiAnalyzeImmediate::ReplaceADDiuSLLWithLUi(InstSeq &Seq) {
...
void OiAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const OiAnalyzeImmediate::InstSeq
&OiAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
