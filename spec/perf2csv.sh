...
for index in ${!NAMES[*]}; do
    ...
    sed -n "/Running ${name} in native mode (small)/,/Running/p" $LOG_TEMP | sed -n "/task-clock/,/blblbl/p" > $LOG_T2
    format_perf_output

    echo -ne "${name}-globals${FIELD_SEP}"
    sed -n "/Running ${name} OpenISA mode (small) with opts -nolocals/,/Running/p" $LOG_TEMP | sed -n "/task-clock/,/blblbl/p" > $LOG_T2
    format_perf_output
